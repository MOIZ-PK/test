...
def main(test_path):
    ...
    args.model_path = 'experiments/pretrained_models/GFPGANCleanv1-NoCE-C2.pth'
    bg_upsampler=None,
    ...
    print(f'Processing My Code ')
    img_list = sorted(glob.glob(os.path.join(test_path, '*')))
    for img_path in [test_path]:
        ...
        for idx, (cropped_face, restored_face) in enumerate(zip(cropped_faces, restored_faces)):
            # save cropped face
            save_crop_path = os.path.join(args.save_root, 'cropped_faces', f'{basename}_{idx:02d}' + ext)
            imwrite(cropped_face, save_crop_path)
            # save restored face
            if args.suffix is not None:
                save_face_name = f'{basename}_{idx:02d}_{args.suffix}' + ext
            else:
                save_face_name = f'{basename}_{idx:02d}.png'
            save_restore_path = os.path.join(args.save_root, 'restored_faces', save_face_name)
            imwrite(restored_face, os.path.join("static/uploads/", 'restore', f'{basename}' + ext))
            # save comparison image
            cmp_img = np.concatenate((cropped_face, restored_face), axis=1)
            imwrite(cmp_img, os.path.join("static/uploads/", 'check', f'{basename}.png'))
        ...
        if restored_img is not None:
            ...
            if args.suffix is not None:
                save_restore_path = os.path.join("static/processed/", 'restored_imgs',
                                                 f'{basename}_{args.suffix}.{extension}')
            else:
                save_restore_path = os.path.join("static/processed/", 'restored_imgs', f'{basename}.{extension}')
